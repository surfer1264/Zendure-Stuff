...
def rpc(ip, method, params=None, timeout=15):
    # ensure_ascii=False ist wichtig: sonst kodiert json.dumps Emojis
    # (Zeichen ausserhalb der BMP, z.B. Akku-/Feuer-Symbole) als
    # \uXXXX-Surrogatpaare. Der schlanke JSON-Parser der Shelly-Firmware
    # (frozen/mjson) kann Surrogatpaare nicht zusammensetzen und lehnt
    # den Wert dann mit "Missing or bad argument 'code'" ab, sobald ein
    # Chunk so ein Zeichen enthaelt. Mit ensure_ascii=False werden die
    # Zeichen direkt als UTF-8-Bytes gesendet, was der Shelly problemlos
    # versteht.
    payload = json.dumps(
