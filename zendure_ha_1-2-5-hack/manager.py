...
class ZendureManager(DataUpdateCoordinator[None], EntityDevice):
    ...
    async def loadDevices(self) -> None:
        ...
        # NEU Joerg Leeres Select – wird gleich mit Sensoren befüllt
        self.p1meter = ZendureRestoreSelect(self, "p1_meter", {0: "sensor.power_actual"},self.update_p1meter_entity)
        ...
        for dev in data["deviceList"]:
            try:
                ...
                # create the device and mqtt server Ihlefeld
        ...
        # NEU Joerg Alle HA-Sensoren als Liste holen und sortieren
        sensor_list = sorted(
            state.entity_id
            for state in self.hass.states.async_all("sensor")
            if state.attributes.get("unit_of_measurement") in ("W", "kW", "watt", "kilowatt")
        )
        self.p1meter.setList(sensor_list)

        # NEU JOERG Startwert aus Konfiguration setzen
        p1 = self.config_entry.data.get(CONF_P1METER, "sensor.power_actual")
        if p1 in sensor_list:
            self.p1meter._attr_current_option = p1
        self.update_p1meter(p1)
                ...
                case ManagerMode.OFF:
                    if len(self.devices) > 0:
                        for d in self.devices:
                            await d.power_off()
    # NEU Jörg
    async def update_p1meter_entity(self, entity: ZendureSelect, _value: Any) -> None:
        """Update p1 meter from select entity change."""
        value = str(entity.current_option)
        _LOGGER.info(f"P1 meter changed to: {value}")
        self.update_p1meter(value)
        self.hass.config_entries.async_update_entry(
            self.config_entry,
            data=self.config_entry.data | {CONF_P1METER: value}
        )  
    ...
    async def power_discharge(self, setpoint: int) -> None:
        ...
        for i, d in enumerate(sorted(self.discharge, key=lambda d: d.electricLevel.asInt, reverse=False)):
            ...
            # make sure we have devices in optimal working range Ihlefeld
            if len(self.discharge) > 1 and i == 0 and d.state != DeviceState.SOCFULL:
                ## self.pwr_low = 0 if (delta := d.discharge_start * 2 - pwr) <= 0 else self.pwr_low + int(delta)
                self.pwr_low = 0 if (delta := d.discharge_start * SmartMode.DISCHARGE_START_FACTOR - pwr) <= 0 else self.pwr_low + int(delta)
